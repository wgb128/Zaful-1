...
+ (PopularCell *)popularCellWithTableView:(UITableView *)tableView IndexPath:(NSIndexPath *)indexPath {
    [tableView registerClass:[PopularCell class] forCellReuseIdentifier:POPULAR_CELL_INENTIFIER];
    return [tableView dequeueReusableCellWithIdentifier:POPULAR_CELL_INENTIFIER forIndexPath:indexPath];
}
...
- (void)setItemsModel:(FavesItemsModel *)itemsModel {
    ...
    if (![itemsModel.sort isEqualToString:@"999"]) {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        topLabel.layer.cornerRadius = 6.0;
        topLabel.layer.masksToBounds = YES;
        [topView addSubview:topLabel];
        
        [topLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.mas_equalTo(nameLabel.mas_trailing).offset(4);
            make.centerY.mas_equalTo(nameLabel.mas_top);
        }];
