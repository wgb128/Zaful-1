...
@implementation PostBagCell

+ (PostBagCell *)postBagCellWithTableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexPath {
    
    [tableView registerClass:[PostBagCell class] forCellReuseIdentifier:NSStringFromClass([PostBagCell class])];
    return [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([PostBagCell class]) forIndexPath:indexPath];
}
...
-(void)setGoodListModel:(GoodListModel *)goodListModel {
    ...
    [self.imgView yy_setImageWithURL:[NSURL URLWithString:goodListModel.goods_img] processorKey:NSStringFromClass([self class]) placeholder:nil options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation progress:nil transform:^UIImage * _Nullable(UIImage * _Nonnull image, NSURL * _Nonnull url) {
        //CGFloat height = 80;
