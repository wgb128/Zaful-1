...
@implementation PostGoodsCell

+ (PostGoodsCell *)postGoodsCellWithTableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexPath {
    
    [tableView registerClass:[PostGoodsCell class] forCellReuseIdentifier:NSStringFromClass([PostGoodsCell class])];
    return [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([PostGoodsCell class]) forIndexPath:indexPath];
}
...
- (void)setGoodsListModel:(ZFCollectionModel *)goodsListModel {
    _goodsListModel = goodsListModel;
    
    [self.imgView yy_setImageWithURL:[NSURL URLWithString:goodsListModel.goods_grid] processorKey:NSStringFromClass([self class]) placeholder:nil options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation progress:nil transform:^UIImage * _Nullable(UIImage * _Nonnull image, NSURL * _Nonnull url) {
        //CGFloat height = 80;
    ...
    if (!goodsListModel.is_promote && !goodsListModel.is_mobile_price) {
