...
@implementation PostOrderCell

+ (PostOrderCell *)postOrderCellWithTableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexPath {
    [tableView registerClass:[PostOrderCell class] forCellReuseIdentifier:NSStringFromClass([PostOrderCell class])];
    return [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([PostOrderCell class]) forIndexPath:indexPath];
}
...
- (void)setGoodsListModel:(PostOrderListModel *)goodsListModel {
    _goodsListModel = goodsListModel;
    
    [self.imgView yy_setImageWithURL:[NSURL URLWithString:goodsListModel.goods_thumb] processorKey:NSStringFromClass([self class]) placeholder:nil options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation progress:nil transform:^UIImage * _Nullable(UIImage * _Nonnull image, NSURL * _Nonnull url) {
        //CGFloat height = 80;
