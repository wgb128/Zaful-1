...
@property (nonatomic,weak) UIButton *integralBtn;//积分按钮
@end

@implementation StyleLikesCell

+ (StyleLikesCell *)styleLikesCellWithTableView:(UITableView *)tableView IndexPath:(NSIndexPath *)indexPath {
    [tableView registerClass:[StyleLikesCell class] forCellReuseIdentifier:STYLE_LIKES_CELL_INENTIFIER];
    return [tableView dequeueReusableCellWithIdentifier:STYLE_LIKES_CELL_INENTIFIER forIndexPath:indexPath];
}

- (void)setReviewsModel:(StyleLikesModel *)reviewsModel {
...
//    NSString *currentDateStr = [dateFormatter stringFromDate:[NSDate dateWithTimeIntervalSince1970:[reviewsModel.addTime integerValue]]];
//    [self.timeLabel setTitle:[NSStringUtils timeLapse:[reviewsModel.addTime integerValue]] forState:UIControlStateNormal];
    ...
    if ([reviewsModel.likeCount isEqualToString:@"0"]) {
        ...
        self.likeNumLabel.text = reviewsModel.likeCount;
    }
    //用户是否点赞
...
//        iconImg.layer.cornerRadius = 20.5;
...
- (void)clickEvent:(UIButton*)sender {
    switch (sender.tag) {
        ...
        {
            if (self.clickEventBlock) {
                self.clickEventBlock(integralBtnTag);
            }
        }
            break;
        case shareBtnTag:
        ...
        {
            if (self.clickEventBlock) {
                self.clickEventBlock(followBtnTag);
            }
        }
            break;
        default:
