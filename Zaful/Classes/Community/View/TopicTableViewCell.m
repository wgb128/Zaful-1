...
+ (TopicTableViewCell *)topicTableViewCellWithTableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexPath {
    //注册cell
    [tableView registerClass:[TopicTableViewCell class] forCellReuseIdentifier:TOPIC_CELL_IDENTIFIER];
    return [tableView dequeueReusableCellWithIdentifier:TOPIC_CELL_IDENTIFIER forIndexPath:indexPath];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(contentImgView.mas_bottom).offset(10);
        }];
...
- (void)setModel:(TopicDetailListModel *)model {
    _model = model;
    //清除子视图防止二次创建
