...
- (void)configUI
{
    ...
    self.cancelButton.frame = CGRectMake(15, [UIScreen mainScreen].bounds.size.height - 50, 90, 25);
    self.cancelButton.titleLabel.textAlignment = NSTextAlignmentLeft;
    [self.view bringSubviewToFront:self.cancelButton];
    [self.view addSubview:self.selectButton];
    self.selectButton.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 90 - 10, self.cancelButton.frame.origin.y, self.cancelButton.frame.size.width, self.cancelButton.frame.size.height);
    self.selectButton.titleLabel.textAlignment = NSTextAlignmentRight;
...
- (UIButton *)selectButton
{
    if (_selectButton == nil) {
        ...
        [_selectButton setTitle:ZFLocalizedString(@"Address_VC_Done", nil) forState:UIControlStateNormal];
