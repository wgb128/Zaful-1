...
-(void)setDatabaseName:(NSString*)fileName
{
    ...
    NSFileManager * fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:PATH_OF_LIBRARY_SUPPORT] == NO)/*001*/ {
        [fileManager createDirectoryAtPath:PATH_OF_LIBRARY_SUPPORT withIntermediateDirectories:YES attributes:nil error:nil];
    }
...
//            }
            ...
            if ([self itemContainsString:tmpSelection withString:item1]) {
                tmpSelection = [tmpSelection stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
            }
            if ([self itemContainsString:tmpGroupBy withString:item1]) {
                tmpGroupBy = [tmpGroupBy stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
            }
            if ([self itemContainsString:tmpOrderBy withString:item1]) {
                tmpOrderBy = [tmpOrderBy stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
                
            }
            
//            if ([tmpSelection rangeOfString:item1].location != NSNotFound) {
//                tmpSelection = [tmpSelection stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
//            }
//            if ([tmpGroupBy rangeOfString:item1].location != NSNotFound) {
//                tmpGroupBy = [tmpGroupBy stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
//            }
//            if ([tmpOrderBy rangeOfString:item1].location != NSNotFound) {
//                tmpOrderBy = [tmpOrderBy stringByReplacingOccurrencesOfString:item1 withString:sqlType.name];
//                
//            }
        }];
    ...
    if (condition.offset > -1 && condition.count > -1) {
        [selectSql appendFormat:@" LIMIT %ld OFFSET %ld ",(long)condition.count,(long)condition.offset];
    }
    return selectSql;
}

- (BOOL)itemContainsString:(NSString *)item withString:(NSString *)str {
    if (![[NSString class] respondsToSelector:@selector(containsString:)])
    {
        //ios 7
        return [item rangeOfString:str].location != NSNotFound;
    }
    else  //for iOS 8
    {
        return [item containsString: str];
    }
}
...
-(id<BCORMEntityProtocol>)translateEntityFromFMResultSet:(FMResultSet*)rs queryCondition:(BCSqlParameter*)condition
{
    ...
    [tmpProperties enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (strcmp(rawPropertyType, @encode(CGFloat)) == 0 ||strcmp(rawPropertyType, @encode(float)) == 0 ||strcmp(rawPropertyType, @encode(double)) == 0 )/*0001*/ {
            ...
            //it's a long
            long  value  = [rs  longForColumn:sqlType.name];
