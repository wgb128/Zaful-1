...
- (void)initPickerHeaderView {
    ...
    CGFloat cancelWidth = [ZFLocalizedString(@"Cancel", nil) textSizeWithFont:[UIFont systemFontOfSize:HeaderButtonTitleFontSize] constrainedToSize:CGSizeMake(200, NSUIntegerMax) lineBreakMode:NSLineBreakByWordWrapping].width;
    CGFloat confirmWidth = [ZFLocalizedString(@"OK", nil) textSizeWithFont:[UIFont systemFontOfSize:HeaderButtonTitleFontSize] constrainedToSize:CGSizeMake(200, NSUIntegerMax) lineBreakMode:NSLineBreakByWordWrapping].width;
    
    CGRect cancelButtonFrame  = CGRectMake(CancelButtonPointX,
                                           CancelButtonPointY,
                                           MAX(cancelWidth, CancelButtonWeight),
                                           CancelButtonHieght);
    
    CGRect confirmButtonFrame = CGRectMake(ConfirmButtonPointX ,
                                           ConfirmButtonPointY,
                                           MAX(confirmWidth, ConfirmButtonWeight),
                                           ConfirmButtonHieght);
    ...
    [confirmButton setTitle:ZFLocalizedString(@"OK", nil) forState:UIControlStateNormal];
...
- (void)confirm {
    [self hide:^{
        if(self.selectedBlock) {
            if (self.isSingleColumn) {
                [NSStringUtils isBlankString:self.selectedItem] ? self.selectedItem = self.dataSource.count > 0 ? self.dataSource[0] : nil : self.selectedItem;
