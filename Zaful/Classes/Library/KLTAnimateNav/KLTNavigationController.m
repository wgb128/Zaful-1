...
@property(nonatomic,strong)NSString *localeLanguageCode;
@end
...
- (void)viewDidLoad {
    ...
    self.localeLanguageCode = [ZFLocalizationString shareLocalizable].nomarLocalizable;
    ...
    _panGestureRec.edges =   [self.localeLanguageCode hasPrefix:@"ar"] ? UIRectEdgeRight : UIRectEdgeLeft;
...
- (void)dragging:(UIPanGestureRecognizer *)pan
{
    ...
    // 得到手指拖动的位移
    CGFloat offsetX = [self.localeLanguageCode hasPrefix:@"ar"] ? -[pan translationInView:self.view].x : [pan translationInView:self.view].x;
    
    // 让整个view都平移     // 挪动整个导航view
    if (offsetX > 0) {
        self.view.transform = [self.localeLanguageCode hasPrefix:@"ar"] ? CGAffineTransformMakeTranslation(-offsetX, 0) : CGAffineTransformMakeTranslation(offsetX, 0);
    }
    ...
    if ([self.localeLanguageCode hasPrefix:@"ar"] ? -offsetX < ScreenWidth : offsetX < ScreenWidth) {
        
        _screenshotImgView.transform =  [self.localeLanguageCode hasPrefix:@"ar"] ? CGAffineTransformMakeTranslation(-(offsetX - ScreenWidth) * 0.6, 0) : CGAffineTransformMakeTranslation((offsetX - ScreenWidth) * 0.6, 0);
    }
...
- (void)dragEnd
{
    ...
    if ([self.localeLanguageCode hasPrefix:@"ar"] ? translateX >= 40 : translateX <= 40) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self.view.transform = [self.localeLanguageCode hasPrefix:@"ar"] ? CGAffineTransformMakeTranslation(-width, 0) : CGAffineTransformMakeTranslation(width, 0);
            // 让imageView位移还原
