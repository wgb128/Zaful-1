...
#define PYTextColor PYSEARCH_COLOR(51, 51, 51)  // 文本字体颜色
...
- (UIButton *)emptyButton
{
    if (!_emptyButton) {
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            emptyButton.py_x = 0.0;
            emptyButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
            emptyButton.titleEdgeInsets = UIEdgeInsetsMake(0.0, emptyButton.imageView.py_width, 0.0, -emptyButton.imageView.py_width);
            emptyButton.imageEdgeInsets = UIEdgeInsetsMake(0.0, - emptyButton.titleLabel.py_width, 0.0, emptyButton.titleLabel.py_width);
        }
    }
    return _emptyButton;
}
...
- (UIView *)searchHistoryTagsContentView
{
    if (!_searchHistoryTagsContentView) {
        ...
        searchHistoryTagsContentView.py_width = self.headerContentView.py_width;
...
- (UILabel *)searchHistoryHeader
{
    if (!_searchHistoryHeader) {
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            titleLabel.py_x = self.headerContentView.py_width - titleLabel.py_width;
        }
    }
...
- (void)setup
{
    ...
    UIBarButtonItem *cancleButtonItem = [[UIBarButtonItem alloc] initWithTitle:[PYSearchCancelButtonText py_localizedString] style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
    [cancleButtonItem setTitleTextAttributes:@{NSForegroundColorAttributeName:ZFCOLOR(51, 51, 51, 1.0),NSFontAttributeName:[UIFont systemFontOfSize:16]} forState:UIControlStateNormal];
    
    
    NSString *localeLanguageCode = [ZFLocalizationString shareLocalizable].nomarLocalizable;
    if ([localeLanguageCode hasPrefix:@"es"]) {
        UIBarButtonItem *negativeSpacer = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
        // -5 代表向右偏移5像素
        negativeSpacer.width = -8;
        self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:negativeSpacer, cancleButtonItem, nil];
    }else{
    ...
    titleView.py_x = 2; // PYSEARCH_MARGIN * 0.3
    titleView.py_y = 7;
    ...
    if ([SystemConfigUtils isRightToLeftShow]) {
        titleLabel.py_x = contentView.py_width - titleLabel.py_width - self.headerContentView.py_x;
    }
    ...
    emptySearchHistoryLabel.font = [UIFont systemFontOfSize:14];
...
- (void)setupHotSearchRectangleTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            rectangleTagLabel.py_x = contentView.py_width - rectangleTagLabel.py_width * (i % PYRectangleTagMaxCol);
        }
        // 添加标签
...
- (void)setupHotSearchRankTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rankTag.font = [UIFont systemFontOfSize:14];
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            rankTag.x = rankView.py_width - rankTag.py_width;
        }
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            rankTextLabel.py_x = CGRectGetMinX(rankTag.frame) - PYSEARCH_MARGIN - rankTextLabel.py_width;
        }
    ...
    for (int i = 0; i < self.rankViews.count; i++) { // 每行两个
        ...
        if ([SystemConfigUtils isRightToLeftShow]) {
            rankView.py_x = contentView.py_width - rankView.py_width * ((i + 1) % 2);
        }
...
- (NSArray *)addAndLayoutTagsWithTagsContentView:(UIView *)contentView tagTexts:(NSArray<NSString *> *)tagTexts;
{
    ...
    for (int i = 0; i < contentView.subviews.count; i++) {
        ...
        if (currentX + subView.py_width + PYSEARCH_MARGIN * countRow > contentView.py_width) { // 得换行
            subView.py_x = [SystemConfigUtils isRightToLeftShow] ? contentView.py_width - subView.py_width : 0;
            subView.py_y = (currentY += subView.py_height) + PYSEARCH_MARGIN * ++countCol;
            currentX = subView.py_width;
            countRow = 1;
        } else { // 不换行
            if ([SystemConfigUtils isRightToLeftShow]) {
                subView.py_x = contentView.py_width - currentX - subView.py_width - PYSEARCH_MARGIN * countRow;
                currentX += subView.py_width;
            } else {
    ...
    switch (searchHistoryStyle) {
        ...
        case PYSearchHistoryStyleBorderTag: // 边框标签
            for (UILabel *tag in self.searchHistoryTags) {
                ...
                // 设置边框颜色
                tag.layer.borderColor = PYSEARCH_COLOR(51, 51, 51).CGColor;
                // 设置边框宽度
...
- (UILabel *)labelWithTitle:(NSString *)title
{
    ...
    label.text = title;
    label.textColor = PYSEARCH_COLOR(51, 51, 51);
