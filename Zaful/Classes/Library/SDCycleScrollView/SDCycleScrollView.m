...
- (void)setupMainView
{
    ...
    if (ISIOS9) {
        mainView.semanticContentAttribute = self.isRightToLeft ? UISemanticContentAttributeForceRightToLeft : UISemanticContentAttributeForceLeftToRight;
    }
...
{
    _imageURLStringsGroup = [SystemConfigUtils isRightToLeftShow] ? [[imageURLStringsGroup reverseObjectEnumerator] allObjects] : imageURLStringsGroup;
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            if (ISIOS9) {
                pageControl.semanticContentAttribute = self.isRightToLeft ? UISemanticContentAttributeForceRightToLeft : UISemanticContentAttributeForceLeftToRight;
            }
...
- (void)automaticScroll
{
    ...
    int targetIndex = [SystemConfigUtils isRightToLeftShow] ? currentIndex - 1 : currentIndex + 1;
    if (targetIndex == _totalItemsCount
        ||([SystemConfigUtils isRightToLeftShow] && targetIndex == 0)) {
        if (self.infiniteLoop) {
            targetIndex = [SystemConfigUtils isRightToLeftShow] ? _totalItemsCount * 0.5 - 1 : _totalItemsCount * 0.5;
        }else{
            return;
        }
...
- (void)layoutSubviews
{
    ...
    if (_pageControlFromBottom == 0) {
        _pageControlFromBottom = 10;
    }
    ...
    if ((_mainView.contentOffset.x == 0
         ||((_mainView.contentOffset.x - (_totalItemsCount - 1) * _mainView.sd_width == 0)
            && [SystemConfigUtils isRightToLeftShow])) &&  _totalItemsCount) {
        int targetIndex = 0;
        if (self.infiniteLoop) {
            targetIndex = [SystemConfigUtils isRightToLeftShow] ? _totalItemsCount * 0.5 - 1 : _totalItemsCount * 0.5;
        }else{
    ...
    CGFloat y = self.mainView.sd_height - size.height - _pageControlFromBottom;//10;
    ...
    [self setCurrentPageWithScrollView:self.mainView];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            ...
            [cell.imageView  yy_setImageWithURL:[NSURL URLWithString:imagePath]
            ...
            cell.imageView.image = [YYImage imageNamed:imagePath];
        }
    ...
    if ([SystemConfigUtils isRightToLeftShow]) {
//        cell.layer.transform = CATransform3DMakeRotation(M_PI, 0, 0.5, 0);
...
{
    [self setCurrentPageWithScrollView:scrollView];
}
...
- (void)setCurrentPageWithScrollView:(UIScrollView *)scrollView {
