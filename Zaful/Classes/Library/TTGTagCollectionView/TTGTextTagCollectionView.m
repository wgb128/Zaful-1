...
@property (nonatomic,strong) NSMutableArray *maxSelectTags;
@end
...
- (void)tagCollectionView:(TTGTagCollectionView *)tagCollectionView didSelectTag:(UIView *)tagView atIndex:(NSUInteger)index {
    ...
    if (_enableTagSelection) {
        if ([self.maxSelectTags containsObject:label]) {
            if (label.selected) {
                label.selected = !label.selected;
                [self resetStyleForLabel:label];
                [self.maxSelectTags removeObject:label];
            }
        }else{
            if (self.maxSelectTags.count >= 6) {
                if ([_delegate respondsToSelector:@selector(showMaxSelectTagsMessage)]) {
                    [_delegate showMaxSelectTagsMessage];
                }
                return;
            }
            label.selected = !label.selected;
            [self resetStyleForLabel:label];
            [self.maxSelectTags addObject:label];
        }
    }
...
-(NSMutableArray *)maxSelectTags {
    if (!_maxSelectTags) {
        _maxSelectTags = [NSMutableArray array];
    }
    return _maxSelectTags;
}
