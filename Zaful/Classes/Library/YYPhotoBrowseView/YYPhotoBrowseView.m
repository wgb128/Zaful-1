...
- (void)setItem:(YYPhotoGroupItem *)item {
    ...
    [_imageView yy_setImageWithURL:item.largeImageURL processorKey:NSStringFromClass([self class]) placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
...
#warning -mark 此处为处理查看大图时，取消的动画效果
...
- (void)longPress:(UILongPressGestureRecognizer *)gesture {
    ...
    if(gesture.state == UIGestureRecognizerStateBegan) {
        if(self.isSaveImage) {
            UIActionSheet *actionsheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:ZFLocalizedString(@"Cancel",nil) destructiveButtonTitle:nil otherButtonTitles:ZFLocalizedString(@"Save_Picture",nil), nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (0 == buttonIndex) {
        NSInteger currentPage = self.currentPage;
        YYPhotoGroupItem *item = _groupItems[currentPage];
        NSData *data = [NSData dataWithContentsOfURL:item.largeImageURL];
