...
- (YYWebImageOperation *)requestImageWithURL:(NSURL *)url
    ...
    if (queue) {
        for (YYWebImageOperation *tempOperation in queue.operations) {
            if ([tempOperation.request.URL.absoluteString isEqualToString:url.absoluteString] && [tempOperation.cacheKey isEqualToString:[NSString stringWithFormat:@"%@-<%@>",url.absoluteString,processorKey]]) {
