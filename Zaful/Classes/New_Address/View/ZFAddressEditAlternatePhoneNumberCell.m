...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (self.addressEditPhoneNumberCheckErrorCompletionHandler) {
        ...
        self.addressEditPhoneNumberCheckErrorCompletionHandler(isError, textField.text);
    }
...
- (void)zfInitView {
    ...
    [self.contentView addSubview:self.alternateLabel];
...
- (void)zfAutoLayoutView {
    [self.alternateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.phoneNumberLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.mas_equalTo(self.contentView.mas_leading).offset(16);
        make.top.mas_equalTo(self.alternateLabel.mas_bottom);
...
- (void)setModel:(ZFAddressInfoModel *)model {
    _model = model;
    self.enterTextField.text = _model.telspare;
    self.phoneNumberLabel.textColor = ZFCOLOR(153, 153, 153, 1.f);
    self.lineView.backgroundColor = ZFCOLOR(221, 221, 221, 1.f);
    [self.lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.mas_equalTo(self.phoneNumberLabel);
    ...
    if (![NSStringUtils isEmptyString:_model.supplier_number_list]) {
        ...
        if ([NSStringUtils isEmptyString:_model.supplier_number_spare]) {
            [self.countryCodeButton setTitle:ZFLocalizedString(@"ModifyAddress_Supplier_Placeholder", nil) forState:UIControlStateNormal];
        } else {
            [self.countryCodeButton setTitle:_model.supplier_number_spare forState:UIControlStateNormal];
...
- (UILabel *)alternateLabel {
    if (!_alternateLabel) {
        _alternateLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        _alternateLabel.font = [UIFont systemFontOfSize:10];
        _alternateLabel.textColor = ZFCOLOR(153, 153, 153, 1.f);
        _alternateLabel.text = ZFLocalizedString(@"ModifyAddress_Alternate_phone", nil);
    }
