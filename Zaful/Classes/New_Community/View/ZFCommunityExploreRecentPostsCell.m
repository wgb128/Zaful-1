...
//
//  ZFCommunityExploreRecentPostsCell.m
//  Zaful
//
...
@interface ZFCommunityExploreRecentPostsCell () <ZFInitViewProtocol>
...
@implementation ZFCommunityExploreRecentPostsCell
...
- (void)followButtonAction:(UIButton *)sender {
    if (self.communityExploreFollowCompletionHandler) {
        self.communityExploreFollowCompletionHandler(self.model);
    }
}

- (void)shareButtonAction:(UIButton *)sender {
    if (self.communityExploreShareCompletionHandler) {
        self.communityExploreShareCompletionHandler(self.model);
    }
}

- (void)likeButtonAction:(UIButton *)sender {
    if (self.communityExploreLikeCompletionHandler) {
        self.communityExploreLikeCompletionHandler(self.model);
    }
}

- (void)reviewButtonAction:(UIButton *)sender {
    if (self.communityExploreReviewCompletionHandler) {
        self.communityExploreReviewCompletionHandler();
    }
}
...
- (void)zfAutoLayoutView {
    ...
    [self.bottomLineView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.shareButton.mas_bottom).offset(12);
        make.bottom.mas_equalTo(self.contentView.mas_bottom).mas_equalTo(-8);
        make.height.mas_equalTo(1);
    }];
}

#pragma mark - setter
- (void)setModel:(ZFCommunityFavesItemModel *)model {
    _model = model;
    ...
    if (![_model.sort isEqualToString:@"999"]) {
    ...
    [cmps enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [content yy_setTextHighlightRange:range color:ZFCOLOR(255, 168, 0, 1.0) backgroundColor:nil tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
            ...
            if (self.communityExploreTopicDetailCompletionHandler) {
                self.communityExploreTopicDetailCompletionHandler(labName);
            }
        }];
    }];
    ...
    if (!_iconImageView) {
        ...
        [_iconImageView addTapGestureWithComplete:^(UIView * _Nonnull view) {
            ...
            if (self.communityExploreUserAccountCompletionHandler) {
                self.communityExploreUserAccountCompletionHandler(self.model.userId);
            }
        }];
    }
...
- (UILabel *)topLabel {
    if (!_topLabel) {
        ...
        _topLabel.font = [UIFont boldSystemFontOfSize:10];
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = ZFCOLOR(237, 237, 237, 1.f);
    }
    return _lineView;
}
...
- (UIView *)bottomLineView {
    if (!_bottomLineView) {
        ...
        _bottomLineView.backgroundColor = ZFCOLOR(237, 237, 237, 1.f);
    }
