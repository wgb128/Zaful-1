...
@interface ZFCommunityFavesListCell () <ZFInitViewProtocol>
...
@implementation ZFCommunityFavesListCell
...
- (void)followButtonAction:(UIButton *)sender {
    if (self.communityFavesFollowCompletionHandler) {
        self.communityFavesFollowCompletionHandler();
    }
}

- (void)shareButtonAction:(UIButton *)sender {
    if (self.communityFavesShareCompletionHandler) {
        self.communityFavesShareCompletionHandler(self.model);
    }
}

- (void)likeButtonAction:(UIButton *)sender {
    if (self.communityFavesLikeCompletionHandler) {
        self.communityFavesLikeCompletionHandler(self.model);
    }
}

- (void)reviewButtonAction:(UIButton *)sender {
    if (self.communityFavesReviewCompletionHandler) {
        self.communityFavesReviewCompletionHandler();
    }
}
...
- (void)setModel:(ZFCommunityFavesItemModel *)model {
    ...
    [cmps enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [content yy_setTextHighlightRange:range color:ZFCOLOR(255, 168, 0, 1.0) backgroundColor:nil tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
            ...
            if (self.communityFavesTopicDetailCompletionHandler) {
                self.communityFavesTopicDetailCompletionHandler(labName);
            }
        }];
    }];
    ...
    if (!_iconImageView) {
        ...
        [_iconImageView addTapGestureWithComplete:^(UIView * _Nonnull view) {
            ...
            if (self.communityFavesUserAccountCompletionHandler) {
                self.communityFavesUserAccountCompletionHandler(self.model.userId);
            }
        }];
    }
