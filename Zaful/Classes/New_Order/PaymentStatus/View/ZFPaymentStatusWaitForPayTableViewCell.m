...
//
//  ZFPaymentStatusCodTableViewCell.m
//  Zaful
//
...
@interface ZFPaymentStatusWaitForPayTableViewCell() <ZFInitViewProtocol>
...
@property (nonatomic, strong) UIButton              *waitForPayButton;
@property (nonatomic, strong) UILabel               *tipsLabel;
@end

@implementation ZFPaymentStatusWaitForPayTableViewCell
...
- (void)zfInitView {
    ...
    [self.containerView addSubview:self.waitForPayButton];
...
- (void)zfAutoLayoutView {
    ...
    [self.waitForPayButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.priceLabel.mas_bottom).offset(20);
        make.height.mas_equalTo(48);
        make.leading.trailing.mas_equalTo(self.lineView);
    }];

    [self.tipsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.waitForPayButton.mas_bottom).offset(20);
        make.leading.trailing.mas_equalTo(self.lineView);
...
- (void)setModel:(ZFOrderCheckDoneDetailModel *)model {
    ...
    switch (model.payState) {
            break;
        case PaymentStateTypeFailure:
        {
            self.statusImageView.image = [UIImage imageNamed:@"combineFailure"];
            self.statusLabel.text = [model.payName isEqualToString:ZFLocalizedString(@"ZFPaymentOnline", nil)] ? ZFLocalizedString(@"ZFPayStateOnlineFailed", nil) : ZFLocalizedString(@"ZFPayStateSMSRequired", nil);
        }
            break;
        case PaymentStateTypeWaite:
        {
            self.statusImageView.image = [UIImage imageNamed:[model.payName isEqualToString:ZFLocalizedString(@"ZFPaymentOnline", nil)] ? @"combineOnlineWait" : @"combineCodWait"];
            self.statusLabel.text = [model.payName isEqualToString:ZFLocalizedString(@"ZFPaymentOnline", nil)] ? ZFLocalizedString(@"ZFPayStateWaiting", nil) : ZFLocalizedString(@"ZFPayStateSMSRequired", nil);
        }
            break;
    }
    ...
    if ([model.payName isEqualToString:ZFLocalizedString(@"ZFPaymentOnline", nil)]) {
        [self.waitForPayButton setTitle:[NSString stringWithFormat:ZFLocalizedString(@"ZFPayStateAuthorize", nil),[ExchangeManager transforPrice:model.order_amount]] forState:UIControlStateNormal];
    }else{
        [self.waitForPayButton setTitle:ZFLocalizedString(@"ZFPayStateSMSVerification", nil) forState:UIControlStateNormal];
    }
    
    if ([model.payName isEqualToString:ZFLocalizedString(@"ZFPaymentOnline", nil)]) {
        self.priceLabel.text = [ExchangeManager transforPrice:model.order_amount];
        self.tipsLabel.text = ZFLocalizedString(@"ZFPayStateOnlineTip", nil);
    }else{
        // 这里cod取整逻辑在后台处理了,直接取值显示
        self.priceLabel.text = [FilterManager changeCodCurrencyToFront:model.multi_order_amount];
        self.tipsLabel.text = ZFLocalizedString(@"ZFPayStateCodTip", nil);
    }
}
...
- (UIButton *)waitForPayButton {
    if (!_waitForPayButton) {
        _waitForPayButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _waitForPayButton.backgroundColor = ZFCOLOR_BLACK;
        _waitForPayButton.userInteractionEnabled = NO;
        [_waitForPayButton setTitleColor:ZFCOLOR(255, 255, 255, 1.f) forState:UIControlStateNormal];
    }
    return _waitForPayButton;
}
